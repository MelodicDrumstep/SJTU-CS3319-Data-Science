...
def run_subject_validation(model_type, data_path='dataset', learning_rate=0.0001, 
    ...
    print(f"Average accuracy for {model_type}: {np.mean(accuracies):.2f}, std: {np.std(accuracies):.2f}")
...
def grid_search(model_type, data_path='dataset', param_grid=None):
    ...
    for params in param_combinations:
        ...
        print(f"Evaluating configuration for {model_type}: {param_dict}")
    ...
    print(f"Best Accuracy for {model_type}: {best_accuracy:.4f}")
...
if __name__ == "__main__":
    ...
    best_params, best_accuracy = grid_search(model_type='DecisionLevelFusion', param_grid=param_grid)
    print(f"Best Hyperparameters: {best_params}, Best Accuracy: {best_accuracy:.4f}")

    best_params, best_accuracy = grid_search(model_type='BiModalDeepAutoencoder', param_grid=param_grid)
    print(f"Best Hyperparameters: {best_params}, Best Accuracy: {best_accuracy:.4f}")